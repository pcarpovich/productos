...
class StockMovementForm(FlaskForm):
    ...
    def validate(self, extra_validators=None):
        ...
        if quantity_val is not None: # Check if quantity has a value (DataRequired should ensure this, but good practice)
            if movement_type in ['Compra']:
                if quantity_val <= 0:
                    self.quantity.errors.append('Quantity must be positive for "Compra".')
                    valid = False
            elif movement_type in ['Pedido operario', 'Entrega cliente']:
                if quantity_val <= 0: # This quantity will be subtracted from stock
                    self.quantity.errors.append('Quantity must be positive for outgoing movements (will be subtracted from stock).')
                    valid = False
            elif movement_type == 'Ajuste':
                if quantity_val == 0:
                    self.quantity.errors.append('Quantity cannot be zero for "Ajuste". Use positive for additions, negative for subtractions.')
                    valid = False
